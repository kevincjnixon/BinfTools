...
#' @param retTerms Boolean if term-document matrix dataframe should be returned. Default=FALSE
...
PathWC<-function(text, cols="Dark2", retTerms=FALSE, minfreq=3,
                           rmwords=c("regulation","process","positive","negative","mediated", "cell",
                                     "cellular", "protein"), title=""){
  text<-sapply(strsplit(text, "%", TRUE),'[[',1)
  docs<-text
