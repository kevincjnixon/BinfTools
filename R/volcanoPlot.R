...
volcanoPlot<-function(res, title, p=NULL, pval=NULL, FC=1, lab=NULL, col=NULL){
  ...
  maxvalx<-max(abs(na.omit(res$log2FoldChange)))
  ...
  text(maxvalx, 0, paste("Total:",(dim(res)[1])+dim(zeroes)[1]), col="purple", pos=2)
