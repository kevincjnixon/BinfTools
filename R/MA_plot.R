...
MA_Plot<-function(res, title, p=NULL, pval=NULL, FC=1, lab=NULL, col=NULL){
  ...
  maxvaly<-max(abs(na.omit(res$log2FoldChange)))
  ...
  numnc<-dim(nc)[1]+dim(subset(zeroes, abs(log2FoldChange)<FC))[1]
  ...
  text(minvalx, -maxvaly, paste("Total:",(dim(res)[1])+dim(zeroes)[1]), col="purple", pos=4)
